...
<body>
  <div class="page-wrapper" style="background-image: url(images/136934.jpg);">
    <!-- Preloader -->
                      ...
                      <div class="panel panel-warning">
                        ...
                        <form action="" method="post">
                          <div class="form-group">
                            <label>Select the Patient Type</label>
                          ...
                          <div class="form-group">
                            <label>Email</label>
                            <input type="email" name="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" title="xyz@something.com" required class="form-control" placeholder="email..." required="" >
                          </div>
                           <div class="form-group">
                         ...
                         <?php
                          ...
                          if(isset($_POST['login']))
                          {
                            ...
                            if ($type=="inpatient")
                            {
                                  ...
                                  $qry=mysqli_query($con,"SELECT * FROM `inpatients`  where email='$email' and password='$pwd'");
                                  $rows=mysqli_num_rows($qry);
                                  $read=mysqli_fetch_array($qry);
                                  if($rows==1)
                                  {
                                    session_start();
                                    $_SESSION['user_id']=$read['p_id'];
                                    $_SESSION['user_type']='inpatient';
                                    $_SESSION['fname']=$read['p_name'];
                                    echo '<script>alert("Welcome '.$read['p_name'].'"); window.location="index.php";</script>';
                                  }
                                  else
                                  {
                                    echo '<script>alert("Login Denied...Try Again"); </script>';
                                  }
                            }
                            elseif ($type=="outpatient") 
                            {
                                ...
                                $qry=mysqli_query($con,"SELECT * FROM user where email='$email' and password='$pwd'");
                                $rows=mysqli_num_rows($qry);
                                $read=mysqli_fetch_array($qry);
                                if($rows==1)
                                {
                                  session_start();
                                  $_SESSION['user_id']=$read['u_id'];
                                  $_SESSION['user_type']='outpatient';
                                  $_SESSION['fname']=$read['first_nm'];
                                  echo '<script>alert("Welcome '.$read['first_nm'].'"); window.location="index.php";</script>';
                                }
                                else
                                {
                                  echo '<script>alert("Login Denied...Try Again"); </script>';
                         ...
                         ?>
                        <center> <a href="registration.php">New user ? Create  new Account </a></center>
                        </div>
